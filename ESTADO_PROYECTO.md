...
- Codex App sin API 2026-05-15: se detecta `codex.exe` de la extension ChatGPT/Codex y se prueba `codex exec` con login ChatGPT. Se añade el flujo `--corregir-prompts-codex-app` y el boton `Corregir con Codex App` en la interfaz: resuelve prompts preparados sin `OPENAI_API_KEY`, genera JSON individuales, importa a `revision_pendiente.csv` y mantiene la subida asistida con revision humana.
