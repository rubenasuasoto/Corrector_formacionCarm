...
- Arranque sin consola 2026-05-25: `Corrector CARM.exe` deja de depender de `iniciar_app_windows.ps1`/PowerShell para abrir el panel y arranca directamente `.venv\Scripts\pythonw.exe interfaz_app.py` con ventana oculta. El arranque de Windows pasa de `Corrector CARM.cmd` a `Corrector CARM.vbs`, conserva compatibilidad para detectar/eliminar el `.cmd` antiguo y el verificador incorpora una comprobacion especifica de arranque sin consola visible.
- Filtro CARM opcionales 2026-05-25: tras ver que CARM mostraba `UD02 - CASO PRÁCTICO 3/4 (OPCIONAL)` con `3 Sin calificar` pero el log los omitía, se refuerza el parser. Se corrige el contador `Enviados`, se detectan filas pendientes por marcadores reales `submissionnotgraded`/`Sin calificar` además de la columna de estado, y si `filter=require_grading` devuelve cero aunque el contador indique pendientes, se reintenta en la vista completa filtrando localmente solo filas sin calificar. Esto cubre cambios de CARM donde la tabla muestra estados pegados como `Enviado para calificarCalificado` o el filtro no lista alumnos aunque el contador sí lo haga.
- Filtro real CARM 2026-05-25: se confirma con HTML de CARM que el valor correcto del desplegable es `filter=requiregrading` y no `require_grading`. Además, Moodle conserva preferencias de tabla por inicial (`tifirst`/`tilast`) aunque la URL ya no las incluya; la automatización resetea preferencias cuando detecta iniciales activas, selecciona `Requiere calificación`, pone tareas por página en `Todos` si está disponible y guarda la tabla antes de leer filas. Prueba real `--solo-listar-carm --actividad ud02cp03`: detecta los 3 pendientes reales y los registra sin descargar archivos.
- Contraste CARM 2026-05-25: se añade `--contrastar-correcciones-carm <csv/json>` para comparar `revision_pendiente.csv` contra la tabla real de `Requiere calificación` sin rellenar formularios, sin guardar y sin limpiar CSV. Prueba real con `C:\temp\vscodec\cursos\1592\temporal\revision_pendiente.csv`: 23/23 filas preparadas siguen pendientes en CARM, 0 sobran del CSV, 8 pendientes aparecen en CARM sin corrección preparada. Informe en `respuestas_extraidas\contraste_carm_revision.json`.
- Retroalimentación importada 2026-05-25: se corrige el caso en el que Codex/API devolvía `Retroalimentación` como un criterio `0/0` en vez de usar el campo `retroalimentacion`. El importador rescata ese comentario como feedback final, elimina ese pseudo-criterio del detalle evaluable y, si un CSV antiguo tiene `retroalimentacion` vacía, la lectura para subida la recupera desde el `.txt` del alumno. Se repara el CSV real del curso `1592`: 6 filas vacías rellenadas desde sus `.txt`, con copia previa `revision_pendiente_backup_feedback_20260525_124816.csv`. `verificar_app.py` añade prueba `Retroalimentacion importada`.
- OCR visual en PDF 2026-05-25: se revisa el caso `ud07cp01` de Juan Marin Rogero. `pypdf` extraía solo el enunciado y omitía el esquema visual, por lo que la API lo calificó como 0. La lectura de PDF ahora detecta páginas con imágenes y, si el OCR aporta contenido adicional sustancial, usa el OCR combinado para incluir diagramas/esquemas. Se recalifica la fila pendiente a `9.0` con feedback correcto y copia previa del CSV `revision_pendiente_backup_juan_ud07cp01_20260525_130549.csv`. Contraste posterior: 1/1 fila preparada coincide en CARM; queda Andrés Martinez Gomez en `ud07cp01` pendiente sin corrección por formato `.pages`.
- Codex App limite 2026-05-25: se revisa un caso en el que `codex exec` alcanzo el limite de uso y, al reintentar, algunos prompts parecian desaparecer. La causa era que un `*_correccion.json` vacio (`{"correcciones":[]}`) se trataba como correccion resuelta y podia archivar el prompt aunque no hubiera nada importable. Ahora API y Codex App solo archivan prompts con JSON importable real, omiten sin gastar tokens los prompts normales sin `Entregas legibles` y dejan esos casos para revision manual. Los prompts explicitos de recorreccion manual (`prompt_*_revision_*.md` o `# Recorreccion manual`) si se consideran corregibles aunque no tengan bloque de entregas legibles. `verificar_app.py` cubre que los JSON vacios no cuenten como resueltos y que las recorrecciones manuales pasen el filtro.
- Codex App conservador 2026-05-26: se corrige el comportamiento tras fallos de Codex/API. Un prompt que ya tiene `*_correccion.json` importable deja de archivarse automaticamente al iniciar otra correccion y tampoco se archiva al finalizar una tanda de Codex/API; queda en `pendientes\prompts_codex` junto a su JSON hasta que el usuario importe la correccion a `revision_pendiente.csv`. Asi, si Codex se queda sin limite a mitad, los prompts ya resueltos se pueden importar, reintentar o pasar por API sin desaparecer. El archivado operativo queda en el flujo de importacion a revision.
- Codex App revision manual 2026-05-26: se corrige la generacion y lectura de prompts sin entregas legibles. Si una actividad solo tiene entregas manuales/ilegibles, ya no se genera un prompt normal vacio; se genera `prompt_<actividad>_revision_manual...md`. Para no perder prompts ya creados, Codex App tambien acepta prompts antiguos con `Entregas legibles: []` y una seccion `Entregas que requieren revision manual` no vacia, tratandolos como recorreccion manual. El workspace de Codex pasa a ser la carpeta del curso para que pueda leer `pendientes` y `codex_project` a la vez.
- Importacion revision manual 2026-05-26: se adapta el importador de JSON de Codex para no fallar cuando una carpeta contiene salidas vacias. Los JSON sin filas importables se omiten con log, y las salidas que Codex devuelva como `revision_manual_necesaria` se transforman en filas bloqueadas de `revision_pendiente.csv` en vez de abortar todo el lote. Si un JSON trae `correcciones: []` y `revision_manual_necesaria: [...]`, se prioriza la lista manual no vacia.
- Revision manual y Pages 2026-05-26: la bandeja de revision manual vuelve a localizar archivos originales aunque ya esten en `pendientes\archivados_prompt`, incluyendo `.pages`, `.numbers` y `.key`, y deja de confundir los `.txt` de correccion con entregas originales. Los documentos `.pages` se intentan leer automaticamente mediante OCR de `preview.jpg`, `preview-web.jpg` o `QuickLook/Preview.pdf`; si se extrae texto suficiente pasan al flujo normal y si no siguen bloqueados para revision humana. Los prompts de recorreccion indican a Codex que busque copias archivadas y que inspeccione la vista previa interna de Pages antes de devolver `revision_manual_necesaria`.
- Regularizacion segura de suspensos 2026-05-27: se anaden `--listar-suspensos-carm` y `--preparar-regularizacion-suspensos-carm`. El flujo entra en la tabla de cada caso practico con filtro `Enviada` (`filter=submitted`), elimina filtros por iniciales, lee la nota desde la celda del boton `Calificacion` (`8,00 / 10,00`), detecta notas ya calificadas menores de 5 y genera `respuestas_extraidas\suspensos_carm_detectados.json` y, si se solicita, `temporal\regularizacion_suspensos_pendiente.csv` con propuesta de nota `5.0` y retroalimentacion vacia. No modifica CARM, no guarda formularios y queda planteado para revision humana/subida asistida posterior.
- Subida asistida de regularizacion 2026-05-27: el CSV de regularizacion incluye el `url_calificador` exacto de cada alumno, por lo que la subida asistida no depende de emparejar nombres si CARM devuelve mojibake. Antes de rellenar valida en el formulario que la nota actual siga siendo menor de 5; si ya no esta suspendido no modifica nada. Para regularizaciones tambien limpia el feedback y desmarca `sendstudentnotifications` u otras casillas de notificacion detectadas, manteniendo el guardado manual del docente.
- Interfaz de regularizacion 2026-05-27: el panel de subida incorpora una tarjeta "Regularizar suspensos" que lanza la preparacion del CSV desde CARM, muestra el estado del archivo `regularizacion_suspensos_pendiente.csv` y permite seleccionarlo como fuente de subida asistida. La validacion de publicacion segura ahora revisa el CSV seleccionado, no solo `revision_pendiente.csv`, para mantener el flujo humano tambien en regularizaciones.
- Revision 2026-05-18: se revisan los avances hechos desde otro equipo. El repo llega ordenado con paquete de usuario final por lista blanca, desinstalador Windows registrado en HKCU, instalador guiado con preparacion opcional de Node.js/Codex CLI y documentacion de instalacion/desinstalacion. Se refuerza `instalar_windows.ps1` para inyectar siempre la carpeta real de `node.exe` en `PATH` antes de ejecutar `codex.cmd`, evitando fallos cuando Node esta instalado pero la terminal aun no refresco variables. Verificacion: `py_compile`, parseo PowerShell de instaladores/lanzadores y `verificar_app.py --instalacion --sin-endpoints` pasan correctamente. Curso activo local detectado en esta revision: `1589`.
- Instalador Python 2026-05-18: el instalador tecnico deja de exigir que Python venga preparado manualmente. Si no detecta Python 3.12+ intenta instalar `Python.Python.3.12` con `winget`, refresca rutas habituales y solo bloquea si no hay Python valido ni `winget`. El instalador guiado informa este estado en la comprobacion previa.
- Instalador sin terminal 2026-05-18: tras prueba en otro ordenador, se elimina la experiencia de terminales visibles durante instalacion/desinstalacion. El `Setup.exe` generado lanza el asistente por PowerShell oculto, el instalador guiado ejecuta la fase tecnica con `CreateNoWindow` y muestra salida/progreso en una ventana propia, y el desinstalador registrado en Windows usa `-WindowStyle Hidden` con progreso grafico. OCR queda como dependencia opcional: si falla, la app sigue funcionando y marca esos documentos para revision manual.
- OCR instalador 2026-05-18: se endurece `instalar_ocr_windows.ps1` tras fallo en equipo limpio. Ahora busca `tesseract.exe` en rutas habituales aunque el `PATH` no este refrescado, inyecta esa ruta en el proceso, instala con `winget --silent` cuando es posible y no considera fatal que falle la descarga del idioma espanol; en ese caso la app sigue instalada y OCR usa ingles si existe o manda la entrega a revision manual.
- Interfaz 2026-05-18: primera pasada de pulido visual y legibilidad. El panel baja la agresividad del registro de actividad, usa superficies mas claras, botones con distribución adaptable y corrige textos visibles sin acentos/mojibake en la pantalla principal, configuración, avisos y confirmaciones.
- Preferencias visuales 2026-05-18: se añade configuración de pantalla con tema `Usar Windows`, `Claro` u `Oscuro`. La preferencia se guarda en `.corrector_app.json`, se aplica al refrescar el panel y queda cubierta por `verificar_app.py`.
- Instalador checklist 2026-05-18: la fase tecnica del instalador guiado deja de ser solo una caja de salida. Ahora muestra un checklist con pasos (`Python`, entorno virtual, dependencias, OCR, Codex, Chromium, lanzador, verificacion, accesos y desinstalador) y una zona de detalle separada, marcando el paso en curso y los avisos sin abrir terminales.
- Configuracion por secciones 2026-05-18: el modal de configuracion deja de ser una columna larga. Ahora usa menu lateral con secciones (`Pantalla`, `Carpetas`, `Curso y automatizacion`, `Windows`, `Estado`, `OpenAI`, `Avanzado`) y panel de contenido, manteniendo los mismos endpoints y controles. `verificar_app.py` comprueba que el menu exista.
- Prueba Codex 2026-05-18: `codex.cmd` oficial queda detectado en `AppData\Roaming\npm`, con sesion ChatGPT activa. La prueba `codex exec --cd C:\temp\vscodec\cursos\1589\codex_project --sandbox read-only` confirma que Codex ve `actividades.json`, `AGENTS.md`, `contexto_didactico.md` y `README.md`. Si se ejecuta desde el sandbox de desarrollo puede fallar al escribir en `C:\Users\ruben\.codex\state_5.sqlite`; fuera del sandbox funciona, que es el caso equivalente al uso real desde la app local.
- Revision rutas Codex 2026-05-18: con `course_scoped_dirs` activo, la base vuelve a quedar en `C:\temp\vscodec\pendientes` y `C:\temp\vscodec\temporal`, mientras el curso activo `1589` usa `C:\temp\vscodec\cursos\1589\pendientes`, `temporal` y `codex_project` como carpetas hermanas. `codex_project` no queda dentro de pendientes ni temporal, y sigue excluido de Git/paquete. Se añade comprobacion offline para detectar bases mal guardadas como `...\cursos\<id>\pendientes` o `...\cursos\<id>\temporal`.
- Instalacion/desinstalacion 2026-05-18: el instalador guiado detecta carpetas existentes, actualiza la app sin borrar `.env`, reutiliza datos/configuracion compatibles y avisa cuando reutiliza carpetas. El desinstalador grafico permite conservar todo, borrar todo o borrar solo partes concretas: pendientes/prompts, temporal/CSV o cursos/cache/proyectos Codex. Por consola quedan disponibles `-EliminarDatos`, `-EliminarPendientes`, `-EliminarTemporal` y `-EliminarCursos`.
- Instalador guiado 2026-05-18: la ventana de instalacion incorpora icono propio y una comprobacion previa visible de Python, dependencias que se prepararan, OCR opcional, carpetas existentes y Node.js/npm/Codex. El instalador avisa expresamente que el modo sin API requiere iniciar sesion una vez con ChatGPT mediante `codex login`; prepara el CLI si puede, pero no intenta iniciar sesion por el usuario.
- Instalador EXE 2026-05-18: se añade `crear_instalador_setup_windows.cmd` / `.ps1` para generar `Corrector_CARM_<version>_Setup_<fecha>.exe`. El EXE es autoextraible: contiene el ZIP limpio generado por `crear_paquete_windows.ps1`, lo extrae en una carpeta temporal y lanza `INSTALAR_CORRECTOR_CARM.cmd`, evitando que el usuario tenga que descomprimir carpetas. El generador compila un bootstrap .NET con `assets/corrector_carm.ico` como icono propio y usa IExpress solo como fallback. Validado en carpeta temporal sin ejecutar la instalacion final; queda pendiente firma digital para evitar avisos de SmartScreen.
- Contexto Codex 2026-05-18: se revisa la diferencia entre API y Codex. La API mantiene prompts optimizados para controlar tokens, mientras que el proyecto Codex por curso exporta ahora un indice (`contexto_didactico.md`), `actividades.json` y `unidades/*.md` con el contenido imprimible completo por unidad. La exportacion sanea texto con mojibake heredado de cache antes de escribirlo y el flujo `codex exec --cd <codex_project>` indica a Codex que consulte esos archivos si el prompt no trae contexto suficiente. Se regeneran los proyectos reales de los cursos `1589` y `1592`.
- Robustez prompts/Codex 2026-05-18: los prompts pasan a incluir metadatos internos `calidad_extraccion` cuando la lectura automatica parece recortada, con mojibake, caracteres no latinos inesperados, OCR limitado o texto fragmentado. Codex recibe la instruccion de verificar el archivo original indicado en `archivo` cuando tenga acceso antes de penalizar, y de no mencionar errores de extraccion en la retroalimentacion salvo que existan tambien en la entrega real del alumno. La verificacion offline cubre caracteres no latinos inesperados y respuestas recortadas.
- Revision manual 2026-05-18: la interfaz añade una bandeja de casos en revision manual. Lee `revision_pendiente.csv`, muestra filas bloqueantes o apartadas, permite guardar una nota docente en `revision_manual_notas.json`, marcar la fila como `revision_manual_necesaria` para que la subida asistida no la publique y generar un prompt `prompt_<actividad>_revision_*.md` para recorreccion con Codex. La prueba offline `Revision manual en interfaz` valida notas, bloqueo y generacion del prompt.
- Subida CARM 2026-05-18: se refuerza el relleno de `Comentarios de retroalimentacion` para reenvios o actividades ya calificadas. La app limpia primero textarea oculto, editor Atto/contenteditable e iframe si existe, vuelve a escribir el feedback nuevo y dispara eventos `input/change/blur`, evitando que Moodle conserve el comentario anterior aunque la nota si cambie.
- Autoprompt 2026-05-18: se corrige la validacion de hora exacta diaria. El patron anterior rechazaba horas `00:00`-`19:59`, por eso parecia que no se guardaba. Ahora acepta `HH:MM` entre `00:00` y `23:59`, rechaza `24:00` y `verificar_app.py` lo cubre con prueba offline.
- Revision profunda 2026-05-18: se revisan scripts, interfaz, instalador, OCR, rutas multicurso, preferencias visuales y documentacion viva. Pasan el parseo PowerShell de instaladores/lanzadores, `py_compile` de Python y `verificar_app.py --instalacion --sin-endpoints`, incluyendo comprobaciones de instalador sin terminal, Python/OCR, menu de configuracion por secciones, tema claro/oscuro/automatico, hora de autoprompt, Codex por curso, revision manual y aislamiento multi-curso. No se regenera `Setup.exe`; debe regenerarse solo cuando se quiera distribuir estos cambios.
- Pantalla/configuracion 2026-05-18: la seccion `Pantalla` incorpora tamaño de letra, altura del registro de actividad, alto contraste y recuerdo de la ultima seccion abierta. La cabecera queda mas limpia: `Actualizar` y `Reiniciar app` pasan a botones compactos con icono, y `Borrar credenciales CARM` se mueve a `Configuracion > Avanzado > Cuenta CARM`. `verificar_app.py` valida las nuevas preferencias.
- Instalador robustez 2026-05-18: tras detectar que el asistente podia cerrarse durante `Instalando dependencias` sin dejar contexto visible, `instalador_guiado_windows.ps1` guarda log en `%TEMP%\Corrector_CARM_instalador.log`, captura errores de la fase tecnica y muestra un dialogo grafico con la ruta del log si algo falla. La ventana de progreso conserva el detalle y `verificar_app.py` comprueba estas protecciones.
- Config instalador 2026-05-18: se corrige la escritura de `.corrector_app.json` desde PowerShell para usar UTF-8 sin BOM. Python pasa a leer la configuracion con `utf-8-sig` para tolerar instalaciones anteriores. Esto evita que al abrir la app se pierdan `pendientes_dir`, `temporal_dir` y `courses_dir`, quedando solo `last_local_url`.
- Setup final 2026-05-18: se corrige el `Setup.exe` autoextraible para no ocultar el asistente grafico, cerrar automaticamente la ventana de progreso al terminar con exito y registrar la salida tecnica en UTF-8 limpio. Se genera e instala `C:\Users\ruben\Desktop\Corrector_CARM_0.3.0-local_Setup_20260518_143503.exe`; la instalacion real crea `.venv`, prepara OCR/Codex/Chromium, conserva rutas de datos en `Documents\Corrector CARM` y pasa `verificar_app.py --instalacion --sin-endpoints`.
- Actualizacion limpia 2026-05-19: el asistente de instalacion detecta una instalacion previa en la carpeta destino, guarda temporalmente `.env` y `.corrector_app.json`, ejecuta el desinstalador antiguo en modo silencioso conservando datos locales y despues instala la nueva version restaurando configuracion compatible. Si falta el desinstalador antiguo, elimina solo la carpeta de app tras verificar que no es el codigo fuente ni un repositorio Git. `verificar_app.py` comprueba esta ruta.
- Suite Moodle 2026-05-19: la idea de una futura suite multi-corrector queda separada en un workspace hermano, `C:\Users\ruben\Desktop\moodle_corrector_suite`. Este proyecto `agente` se mantiene como app del Corrector CARM; la suite solo referencia la arquitectura y contratos futuros, sin mezclar codigo operativo ni datos locales.
...
- Instalador y Codex 2026-05-15/17: el asistente guiado añade la opcion de preparar Codex sin API. El instalador tecnico acepta `-PrepararCodex`, no usa el `codex.exe` de la extension ChatGPT/Codex de VS Code, y prepara el CLI oficial instalado con npm cuando es posible. Si no queda disponible, la app avisa y mantiene API/modo manual.
...
- Robustez instalacion 2026-05-16: la interfaz ya no lanza `corrector_agente.py` con el Python global si existe `.venv\Scripts\python.exe`. Esto evita que el login CARM falle con `Playwright no esta disponible` cuando el panel se abre accidentalmente fuera de la venv. El estado local sigue avisando de que conviene reiniciar con `iniciar_app_windows.cmd`.
- Desinstalacion Windows 2026-05-17: se anaden `desinstalar_windows.cmd` / `.ps1` y el instalador registra `Corrector CARM` en `Aplicaciones instaladas` mediante HKCU. La desinstalacion elimina app, accesos, arranque y registro; por defecto conserva datos locales y solo los borra con `-EliminarDatos`.
- Endurecimiento distribucion 2026-05-17: el desinstalador deja de borrar carpetas padre de datos y solo elimina rutas configuradas concretas (`pendientes`, `temporal`, `cursos`) cuando se usa `-EliminarDatos`. Si Windows no permite consultar procesos, avisa sin abortar la desinstalacion. El paquete guiado anade un `LEEME_INSTALACION.txt` mas claro para distinguir entradas de usuario, mantenimiento y diagnostico.
- Orden del proyecto 2026-05-17: se actualiza `ARQUITECTURA_PROYECTO.md` con la decision de no mover aun lanzadores Windows fuera de raiz para no romper instaladores ni accesos directos. La limpieza profesional queda planteada como fase gradual: paquete claro ahora, refactor de carpetas con wrappers despues.
- Paquete usuario final 2026-05-17: `crear_paquete_windows.ps1` deja de empaquetar `git ls-files` completo y pasa a lista blanca. El ZIP incluye solo runtime, instalador, reparacion, diagnostico, plantilla `.env.example`, prompts, iconos e instrucciones de usuario generadas. Quedan fuera `AGENTS.md`, estado interno, arquitectura, roadmap, checklist de release, `.gitignore`, `preparar_release*`, secretos y artefactos generados.
- Instalador Codex 2026-05-17: la opcion de Codex del instalador deja de ser solo deteccion. Ahora detecta Node.js/npm, instala Node.js LTS con `winget` si falta, instala/actualiza `@openai/codex` con npm y comprueba `codex login status`. Si falla, no aborta la instalacion principal: la app sigue disponible con API o modo prompt manual.
- Verificacion profunda 2026-05-17: se ejecuta `python -m py_compile corrector_agente.py interfaz_app.py verificar_app.py preparar_release.py`, parseo PowerShell de instaladores/lanzadores y `.\.venv\Scripts\python.exe verificar_app.py --instalacion`. Resultado OK. La prueba offline `Cache SQLite CARM` pasa correctamente, cubriendo el fallo historico de consultas SQLite/placeholders. Tambien se genero un paquete de usuario final de prueba de 33 archivos y se comprobo que no incluye `.env`, `.venv`, cache, logs, respuestas, correcciones, `__pycache__` ni documentos internos de desarrollo. No se ejecuto instalacion real de Node/Codex en el equipo de desarrollo para no modificar el entorno global; queda implementada en el instalador.
