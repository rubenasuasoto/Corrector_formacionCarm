...
- Setup final 2026-05-18: se corrige el `Setup.exe` autoextraible para no ocultar el asistente grafico, cerrar automaticamente la ventana de progreso al terminar con exito y registrar la salida tecnica en UTF-8 limpio. Se genera e instala `<workspace>\Corrector_CARM_0.3.0-local_Setup_20260518_143503.exe`; la instalacion real crea `.venv`, prepara OCR/Codex/Chromium, conserva rutas de datos en `Documents\Corrector CARM` y pasa `verificar_app.py --instalacion --sin-endpoints`.
...
- Suite Moodle 2026-05-19: la idea de una futura suite multi-corrector queda separada en un workspace hermano, `<workspace>\moodle_corrector_suite`. Este proyecto `agente` se mantiene como app del Corrector CARM; la suite solo referencia la arquitectura y contratos futuros, sin mezclar codigo operativo ni datos locales.
